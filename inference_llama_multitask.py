...
from llama import MultiTaskFunctionLM
from inference_modes_multitask import func_embedding_inference, kamel_embedding_inference, vh_embedding_inference
...
def load(model_uri: str, local_rank: int, world_size: int, func_dicts: List[dict], func_load_path: str) -> MultiTaskFunctionLM:
    ...
    funcmodel = MultiTaskFunctionLM(model, tokenizer, func_dicts = func_dicts, load_path=func_load_path)
...
def main(model_uri: str, modes: List[str], datasets: List[str], logits_biases: List[float], dataset_to_inference: int, temperature: float = 0, top_p: float = 0.95, return_top: int = 5, func_load_path: str = "None", st_idx=0, ed_idx=10000, suffix=""):
    # set random seed
    torch.manual_seed(1)
    ...
    dataset = datasets[dataset_to_inference]
    mode = modes[dataset_to_inference]
    logits_bias = logits_biases[dataset_to_inference]
    ...
    for d in datasets:
        ...
        elif d.startswith("kamel"):
            n_first = int(d.split("_")[-1])
            func_dict = json.load(open("data/kamel/func_dict.json"))
            func_dict = {f"<{k}>": v for k, v in func_dict.items()}
            func_dict = {k: v for k, v in func_dict.items() if v < n_first}
        else:
            raise NotImplementedError(f"Unknown dataset: [{d}]")
    ...
    funcmodel = load(model_uri, local_rank, world_size, func_load_path=func_load_path, func_dicts=func_dicts)
    ...
    if local_rank == 0:
        ...
        output_file = f"{output_dir}/multitask-inference-{size}-{func_model_name}-{mode}-{dataset}-bias_{logits_bias}{suffix}.jsonl"
    ...
    for case_idx, question in tqdm(enumerate(test_cases), total=len(test_cases)):
        ...
        if mode == "func_embedding":
            log = func_embedding_inference(templates, case_idx, question, funcmodel, dataset_to_inference, temperature, top_p, max_gen_len, return_top)
        elif mode == "vh_embedding_inference":
            log = vh_embedding_inference(case_idx, question, funcmodel, dataset_to_inference, temperature, top_p, max_func_call)
        elif mode == "kamel_embedding_inference":
            # print(json.dumps(templates, indent=2))
            # print(case_idx)
            # print(question)
            log = kamel_embedding_inference(templates, case_idx, question, funcmodel, dataset_to_inference, temperature, top_p, max_gen_len, max_func_call)
...
if __name__ == "__main__":
    ...
    mode1 = "kamel_embedding_inference"
    dataset1 = "kamel_234"
    logits_bias1 = 10

    mode2 = "func_embedding"
    dataset2 = "gsm8k-xl"
    logits_bias2 = 3.0

    func_load_path = "checkpoints/kamel-gsm8k_xl-multi-debug/epoch_0.pth"
    # NOTE: Please make sure that the order of dataset is the same as the one in the multi-task training
    modes = [mode1, mode2]
    datasets = [dataset1, dataset2]
    logits_biases = [logits_bias1, logits_bias2]
    dataset_to_inference = 0        # KAMEL dataset. Other indices won't matter, but are required for initializing the multi-task model
    ...
    main(
        ...
        logits_biases = logits_biases,
        dataset_to_inference = dataset_to_inference,
    )
